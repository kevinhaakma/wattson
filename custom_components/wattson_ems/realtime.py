...
class ExportRecovery:
    ...
    async def _apply(self) -> None:
        ...
        try:
            ...
            # ook plan-ontladen-aborts tellen als bronexport voor de
            # exportrust-gate: assist ziet die windows zelf niet (mode-gate)
            c.assist.export_seen = time.monotonic()
...
class AssistController:
    ...
    def __init__(self, c) -> None:
        ...
        self.export_seen: float | None = None  # laatste waargenomen bronexport
    ...
    async def apply(self) -> None:
        ...
        # exportrust-geheugen voor de startgate van piek-ontladen. Tijdens
        # assist-ontladen leest source_p1 het accuvermogen terug en kan hij
        # dus geen valse export melden; in rust en tijdens laden is elke
        # bronexport een teken dat het zonoverschot nog rond nul pendelt.
        if source_p1 is not None and source_p1 < -DISCHARGE_EXPORT_ABORT_W:
            self.export_seen = time.monotonic()
        ...
        if c.assist_active == "ontladen" and (
            ...
            if not A.export_quiet(time.monotonic(), self.export_seen,
                                  ASSIST_DISCHARGE_QUIET_S):
                _LOGGER.debug(
                    "Wattson assist: piek %.0f W maar bronexport <%ds geleden"
                    " — wacht op exportrust", p1, ASSIST_DISCHARGE_QUIET_S)
                return
            applied = await c.set_battery(
